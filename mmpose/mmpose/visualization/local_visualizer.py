...
class PoseLocalVisualizer(OpencvBackendVisualizer):
    ...
    def __init__(self,
                 ...
                 cobb_path: Optional[str] = None,
                 config_name: Optional[str] = None,  # 读取训练配置文件的名称
                 draw_bbox: bool = True,
        ...
        self.config_name = config_name or self._get_config_name()  # 自动读取配置文件名
    ...
    def _get_config_name(self):
        ...
        for arg in sys.argv:
            try:
                ...
                # 验证路径特征
                path_str = str(arg_path).replace('\\', '/').lower()
                if all(marker in path_str for marker in CONFIG_MARKERS):
                    config_candidates.append(arg_path)
        ...
        if config_candidates:
            ...
            # 保留原始文件名大小写和特殊字符
            config_name = selected_config.stem
            print(f"[INFO] Detected config: {config_name} (path: {selected_config})")
            return config_name

        print("[WARNING] Using default config name")
    ...
    def _draw_instances_kpts(self,
        ...
        注意:
            1. 当kpt_thr设为负值时，将显示所有关键点，不进行阈值过滤
            2. skeleton_style可选'mmpose'或'openpose'两种风格
            3. 如果提供了cobb_path，会同时计算和保存Cobb角度
        """

        # 如果使用openpose风格，调用对应的绘制函数
        if skeleton_style == 'openpose':
        ...
        if 'keypoints' in instances:
            # 获取关键点坐标，优先使用transformed_keypoints
            keypoints = instances.get('transformed_keypoints',
                                      instances.keypoints)

            # 获取关键点可见性信息，如果没有则默认全部可见
            ...
            for kpts, visible in zip(keypoints, keypoints_visible):
                ...
                if self.skeleton is not None and self.link_color is not None:
                    ...
                    for sk_id, sk in enumerate(self.skeleton):
                        ...
                        # 准备连接线的坐标和样式
                        X = np.array((pos1[0], pos2[0]))  # 横坐标
                        Y = np.array((pos1[1], pos2[1]))  # 纵坐标
                        # 处理颜色格式
            ...
            # 如果提供了cobb_path，计算并保存Cobb角度
            if hasattr(self, 'cobb_path') and self.cobb_path is not None:
                # 准备关键点数据
                kp_list = []
                for kp, vs in zip(keypoints[0], keypoints_visible[0]):
                    # 将关键点坐标和可见度合并
                    kp_item = np.append(kp, vs)
                    kp_list.append(kp_item)

                # 获取图像文件名
                img_name = image_path.split('/')[-1]

                # 计算Cobb角
                kp_list = np.array(kp_list).reshape(-1)
                cobb = cobb_caculate(kp_list)
                print(f"Cobb calculation result: {cobb}")  # 添加日志
                cobb_str = str(cobb) + '°'
                cobb_result = {img_name: cobb_str}

                # 将结果转换为JSON格式
                json_string = json.dumps(cobb_result)

                # 生成保存路径并创建目录
                json_save = os.path.join(self.cobb_path, 'cobb', self.config_name)
                print(f"Saving Cobb results to: {json_save}")  # 添加日志
                if not os.path.exists(json_save):
                    os.makedirs(json_save)
                    
                # 保存结果到JSON文件
                json_file_path = os.path.join(json_save, os.path.splitext(img_name)[0] + '.json')
                with open(json_file_path, 'w', encoding='utf-8') as json_file:
                    json_file.write(json_string)
                print(f"Saved Cobb result to: {json_file_path}")  # 添加日志
        ...
        if draw_gt:
            ...
            if 'gt_instances' in data_sample:
                gt_img_data = self._draw_instances_kpts(
                    gt_img_data, data_sample.img_path, data_sample.gt_instances, kpt_thr,
        ...
        if draw_pred:
            ...
            if 'pred_instances' in data_sample:
                pred_img_data = self._draw_instances_kpts(
                    pred_img_data, data_sample.img_path, data_sample.pred_instances, kpt_thr,
                    show_kpt_idx, skeleton_style)
                if draw_bbox:
